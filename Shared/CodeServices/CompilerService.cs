...
namespace BlazorApp.Shared.CodeServices
{
    public partial class CompilerService
    {
        ...
        public async Task<bool> SubmitSolution(string code, IEnumerable<MetadataReference> references, string testAgainst = "true")
        {
            ...
            return CodeOutput.Contains(testAgainst) || CodeOutput == testAgainst;
        }
        ...
        public async Task RunSubmission(string code)
        {
            ...
            {
                InfoOutput += $@"<br /><span class=""error"">Catch: {HttpUtility.HtmlEncode(CSharpObjectFormatter.Instance.FormatException(ex))}</span>";
                await CodeOutputChanged.InvokeAsync($"PARSE ERROR {HttpUtility.HtmlEncode(CSharpObjectFormatter.Instance.FormatException(ex))}");
            }
            finally
            {
                Console.SetOut(previousOut);
